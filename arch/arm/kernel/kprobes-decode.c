...
static void __kprobes simulate_ldm1stm1(struct kprobe *p, struct pt_regs *regs)
{
	...
	int reg_bit_vector;
	int reg_count;	
	unsigned long inst[2] = {0,0};

	access_process_vm(current, regs->ARM_sp - 8, (void *)&inst, sizeof(inst), 0);
	...
	if (wbit) {
		...
		access_process_vm(current, addr, (void *)&inst, sizeof(inst), 0);
	}
}
...
static void __kprobes emulate_str(struct kprobe *p, struct pt_regs *regs)
{
	...
	/*--------------------------------------*
	  ...
	  * In some cases, the 
	  * 	 mov	 pc, %[fn]
	  * instruction in insnslot_3arg_rflags() function may jump 
	  * to the address pointed by p->addr instead of i_fn for 
	  * unknown reasons.
	  * It may trap the thread before the thread returns from 
	  * an undefined instruction trap, and causes processor mode
	  * errors (e.g. executes user space codes in privilege mode).
	  * A tricky solution provided here is to modify the value
	  * of p->addr to the memory address in front of the code that calls 
	  * insnslot_3arg_rflags() before the thread invokes 
	  * insnslot_3arg_rflags(), and restores the value of p->addr 
	  * after the thread returns from insnslot_3arg_rflags().
	  */
	 __asm__ __volatile__ (
			 "str	 pc, %[addr]	 \n\t"
			 : [addr] "+m" (p->addr)
	...
	/*--------------------------------------*/
	/* added by Kevin */
	/* restores the value of p->addr */
	p->addr = ( kprobe_opcode_t * ) iaddr;
	/*--------------------------------------*/
}
...
emulate_generic_r0_12_noflags(struct kprobe *p, struct pt_regs *regs)
{
    ...
    register void *rfn asm("lr") = p->ainsn.insn;
...
space_cccc_100x(kprobe_opcode_t insn, struct arch_specific_insn *asi)
{
    ...
    if (reglist & 0x8000)
    {
        handler = is_ldm ? simulate_ldm1stm1 : simulate_stm1_pc;
    }
	else
	{
...
#ifdef UPROBE_PATCH
enum kprobe_insn __kprobes
arm_uprobe_decode_insn(kprobe_opcode_t insn, struct arch_specific_insn *asi)
{
	asi->insn_check_cc = condition_checks[insn>>28];
	asi->insn[1] = KPROBE_RETURN_INSTRUCTION;
